...
@section('title', 'Tambah Foto Background Hero')
@section('content')
        ...
        <div>
            <div class="text-muted small mb-1">
                <a href="{{ route('admin.banner.index') }}" class="text-decoration-none text-muted">
                    <i class="fa-solid fa-panorama me-1"></i> Background Hero
                </a>
                <i class="fa-solid fa-chevron-right mx-2 text-muted small"></i>
                <span class="text-dark font-mono">Tambah Foto</span>
            </div>
            <h4 class="fw-bold mb-0" style="color:var(--primary);">Tambah Foto Background Baru</h4>
        </div>
    ...
    <form action="{{ route('admin.banner.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mt-4 d-flex align-items-center justify-content-end gap-2 p-3 bg-white border rounded-4 shadow-sm">
            ...
            <button type="submit" class="btn btn-primary px-5 fw-bold shadow-sm">
                <i class="fa-solid fa-floppy-disk me-1.5"></i> Simpan Foto
            </button>
        </div>
    </form>
