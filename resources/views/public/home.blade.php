...
@push('styles')
    <style>
        ...
        /* ── Hero Background Slideshow (Rotates every 5s) ── */
        .hero-bg-slider {
            position: absolute;
            inset: 0;
            z-index: 0;
        ...
        .unggulan-scroll-wrap::-webkit-scrollbar {
            height: 4px;
        }

        .unggulan-scroll-wrap::-webkit-scrollbar-track {
        ...
        .ug-overlay {
            background: linear-gradient(180deg,
                    ...
                    /* sebelumnya 0.97 — foto sedikit lebih terlihat di bawah */
                );
        }
        ...
        @keyframes pinPulse {
            ...
            50% {
                box-shadow: 0 4px 28px rgba(200, 155, 60, 1);
            }
        }
...
@section('content')
    ...
    <section class="hero-section" id="hero">
        <!-- Background Image Slideshow (Rotates every 5s) -->
        <div class="hero-bg-slider" id="heroBgSlider">
            @if (isset($banners) && $banners->count() > 0)
                @foreach ($banners as $index => $banner)
                    <div class="hero-bg-slide {{ $index === 0 ? 'active' : '' }}"
                        style="background-image: url('{{ Storage::url($banner->gambar) }}');"></div>
                @endforeach
            @else
            ...
            <div class="row align-items-center gy-5 py-5">
                ...
                <div class="col-lg-8 col-xl-7">
                    ...
                    <p class="hero-subtitle mb-5">
                        Temukan destinasi wisata menakjubkan, event menarik, dan informasi pariwisata lengkap Kabupaten
                        Magetan dalam satu platform.
                    </p>
                    ...
                    @php
                        $heroStats = [
                            [
                                ...
                                'num' => App\Models\Wisata::count(),
                                'label' => 'Destinasi Wisata',
                            ],
                            [
                                'icon' => 'fa-calendar-days',
                                'num' => App\Models\Event::count(),
                                'label' => 'Event & Agenda',
                            ],
                        ];
                    @endphp
                ...
                <div class="d-flex justify-content-between align-items-end mb-5">
                    <div data-aos="fade-up">
                        <div class="section-badge"
                            style="background:rgba(200,155,60,0.15);color:#C89B3C;border:1px solid rgba(200,155,60,0.3);">
                        ...
                        <p class="mb-0" style="color:rgba(255,255,255,0.6);">Wisata pilihan kami yang wajib Anda kunjungi
                        </p>
                    </div>
                    <a href="{{ route('public.wisata') }}"
                        class="btn-interactive btn-interactive-gold btn-interactive-md d-none d-md-inline-flex"
                        data-aos="fade-left">
                ...
                <div class="unggulan-scroll-wrap" data-aos="fade-up" data-aos-delay="100">
                    @foreach ($wisataPinned as $wp)
                        <div class="ug-card">
                            {{-- Image --}}
                            @if ($wp->thumbnail)
                                <img class="ug-img" src="{{ Storage::url($wp->thumbnail) }}" alt="{{ $wp->nama }}"
                                    loading="lazy">
                            @else
                            ...
                            <div class="ug-content">
                                ...
                                <div class="ug-footer">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        @endif
                                        @if ($wp->harga_tiket > 0)
                                            <span class="ug-price">Rp
                                                {{ number_format($wp->harga_tiket, 0, ',', '.') }}</span>
                                        @else
                                            <span class="ug-price" style="color:rgba(255,255,255,0.6);">Gratis</span>
                                        @endif
                                    </div>
                                    <a href="{{ route('public.wisata.detail', $wp->slug) }}"
                                        class="btn-interactive btn-interactive-card btn-interactive-sm">
                            ...
                            {{-- Full card hitbox --}}
                            <a href="{{ route('public.wisata.detail', $wp->slug) }}" class="ug-hitbox"
                                aria-label="{{ $wp->nama }}"></a>
                        </div>
                    @endforeach
                </div>
    ...
    <section class="section-py">
        <div class="container">
            <div class="d-flex justify-content-between align-items-end mb-5">
                ...
                <a href="{{ route('public.wisata') }}"
                    class="btn-interactive btn-interactive-forest btn-interactive-md d-none d-md-inline-flex"
                    data-aos="fade-left">
                    <span class="btn-text-initial">Lihat Semua</span>
                    <div class="btn-text-hover">
                        <span>Lihat Semua</span>
                    ...
                    @foreach ($wisata as $w)
                        <div class="col-md-6 col-xl-4" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                            ...
                            <div class="wisata-card-3d" data-tilt>
                                ...
                                {{-- Full-bleed image --}}
                                @if ($w->thumbnail)
                                    <img class="wc-img" src="{{ Storage::url($w->thumbnail) }}"
                                        alt="{{ $w->nama }}" loading="lazy">
                                @else
                                ...
                                <div style="position:absolute;top:14px;right:14px;z-index:10;">
                                    @auth
                                        <button type="button"
                                            ...
                                            <i
                                                class="fa-heart {{ auth()->user()->wishlist->contains($w->id) ? 'fa-solid' : 'fa-regular' }}"></i>
                                        </button>
                                    @else
                                        <a href="{{ route('login') }}" class="wishlist-btn wishlist-btn-guest"
                                            title="Login untuk menyukai wisata" aria-label="Login untuk menyukai">
                                ...
                                <a href="{{ route('public.wisata.detail', $w->slug) }}"
                                    class="position-absolute inset-0 w-100 h-100" style="z-index:3;"
                                    aria-label="{{ $w->nama }}"></a>
                            </div>
                        </div>
                    @endforeach
            ...
            document.querySelectorAll('[data-tilt]').forEach(card => {
                ...
                function resetCard() {
                    card.style.transition =
                        'transform 0.5s cubic-bezier(0.2, 0.6, 0.2, 1), box-shadow 0.4s cubic-bezier(0.2,0.6,0.2,1)';
                    ...
                    @foreach ($events as $event)
                        <div class="col-md-6 col-xl-3" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                            ...
                            <div class="wisata-card-3d event-card-3d" data-tilt>
                                @if ($event->poster)
                                    <img class="wc-img" src="{{ Storage::url($event->poster) }}"
                                        alt="{{ $event->judul }}" loading="lazy">
                                @else
                                ...
                                <div class="d-flex justify-content-between align-items-start"
                                    style="position:absolute;top:16px;left:16px;right:16px;z-index:3;">
                                    <div
                                        style="background:var(--accent);color:var(--primary-dark);border-radius:12px;padding:6px 12px;text-align:center;box-shadow:0 4px 14px rgba(0,0,0,0.3);backdrop-filter:blur(8px);">
                                        <div
                                            style="font-size:1.15rem;line-height:1;font-weight:800;font-family:'Plus Jakarta Sans',sans-serif;">
                                            {{ $event->tanggal->format('d') }}</div>
                                        <div
                                            style="font-size:0.65rem;text-transform:uppercase;letter-spacing:0.5px;font-weight:700;">
                                            {{ $event->tanggal->format('M Y') }}</div>
                                    </div>
                                ...
                                <div class="wc-bottom flex-column align-items-start" style="gap:6px;">
                                    <p class="wc-name text-white fw-bold mb-1"
                                        style="font-size:1.05rem;line-height:1.3;text-shadow:0 2px 8px rgba(0,0,0,0.7);">
                                        {{ Str::limit($event->judul, 45) }}
                                    </p>
                                    <div class="d-flex align-items-center gap-2 flex-wrap">
                                        <span class="wc-cat"
                                            style="background:rgba(15,34,26,0.85);backdrop-filter:blur(8px);font-size:0.65rem;">
                                            <i
                                                class="fa-solid fa-location-dot me-1 text-warning"></i>{{ Str::limit($event->lokasi, 22) }}
                                        </span>
                                        @if ($event->jam)
                                            <span class="wc-price"
                                                style="background:rgba(10,22,18,0.75);font-size:0.65rem;">
                                ...
                                <a href="{{ route('public.event.detail', $event->id) }}"
                                    class="position-absolute inset-0 w-100 h-100" style="z-index:4;"
                                    aria-label="{{ $event->judul }}"></a>
                            </div>
                        </div>
                    @endforeach
                </div>
                    ...
                    @foreach ($berita as $b)
                        <div class="col-md-4" data-aos="fade-up" data-aos-delay="{{ $loop->iteration * 100 }}">
                            ...
                            <div class="wisata-card-3d berita-card-3d" data-tilt>
                                @if ($b->thumbnail)
                                    <img class="wc-img" src="{{ Storage::url($b->thumbnail) }}"
                                        alt="{{ $b->judul }}" loading="lazy">
                                @else
                                    <div class="wc-nophoto" style="background:linear-gradient(135deg,#142e20,#0a1b13);">
                                ...
                                <div class="d-flex justify-content-between align-items-center"
                                    style="position:absolute;top:16px;left:16px;right:16px;z-index:3;">
                                    <span class="wc-cat"
                                        style="background:rgba(26,107,58,0.85);backdrop-filter:blur(8px);border:1px solid rgba(255,255,255,0.2);">
                                ...
                                <div class="wc-bottom flex-column align-items-start" style="gap:6px;">
                                    <p class="wc-name text-white fw-bold mb-1"
                                        style="font-size:1.1rem;line-height:1.35;text-shadow:0 2px 8px rgba(0,0,0,0.7);">
                                        {{ Str::limit($b->judul, 60) }}
                                    </p>
                                    <p class="mb-0 text-white-50 small"
                                        style="font-size:0.75rem;line-height:1.4;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-shadow:0 1px 4px rgba(0,0,0,0.7);">
                                        {{ Str::limit(strip_tags($b->isi), 90) }}
                                    </p>
                                </div>

                                <a href="{{ route('public.berita.detail', $b->id) }}"
                                    class="position-absolute inset-0 w-100 h-100" style="z-index:4;"
            ...
            <div class="cta-aurora-card" data-aos="zoom-in" data-aos-duration="900">
                ...
                <div class="row align-items-center position-relative g-4" style="z-index: 2;">
                    ...
                    <div class="col-lg-7 pe-lg-4">
                        ...
                        <p class="cta-desc">
                            Temukan keindahan Telaga Sarangan, segarnya udara lereng Gunung Lawu, kekayaan tradisi budaya,
                            dan nikmati aneka ragam kuliner legendaris khas Magetan dalam satu genggaman.
                        </p>
                        ...
                        <div class="d-flex gap-3 flex-wrap align-items-center mb-4">
                            ...
                            <a href="{{ route('public.tentang') }}" class="btn-cta-secondary">
                                ...
                                <span>Tentang Pesona Magetan</span>
                            </a>
                        </div>

                        <div class="d-inline-flex align-items-center gap-2 px-3 py-2"
                            style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.12); border-radius:100px; font-size:0.8rem; color:rgba(255,255,255,0.85); backdrop-filter:blur(8px);">
